    ...
    for(uint32 skt = 0; skt < m->getNumSockets(); ++skt)
    {
        ...
        for (size_t p = 0; p < m->getNumCXLPorts(skt); ++p)
        {
            ...
            else
            {
                ...
                md.CXLMEM_Wr_socket_port[skt][p] = CXLBWWrScalingFactor * toBW(getCXLCMCounter((uint32)p, PCM::EventPosition::CXL_RxC_MEM, uncState1[skt], uncState2[skt])); // wxx
                // md.CXLMEM_Wr_socket_port[skt][p] = CXLBWWrScalingFactor * toBW(getCXLCMCounter((uint32)p, PCM::EventPosition::CXL_TxC_MEM, uncState1[skt], uncState2[skt])); // wxx
            }
